...
export const AppBar = () => {
  ...
  return <header className="w-full bg-gradient-to-r from-pink-600 via-pink-500 to-rose-500 shadow-2xl sticky top-0 z-50 backdrop-blur-sm">
      ...
      <nav className="flex items-center justify-between px-3 py-3 sm:px-6 sm:py-4 md:py-5 max-w-7xl mx-auto relative">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-pink-700/10 via-transparent to-pink-700/10 rounded-lg bg-pink-600"></div>
        
        <div className="flex items-center gap-2 sm:gap-4 cursor-pointer flex-1 min-w-0 group relative z-10" onClick={() => navigate("/")}>
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 relative z-10">
          ...
          {/* Mobile menu button */}
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden relative p-2 rounded-xl hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-white/50 focus:outline-none text-white transition-all duration-300 group backdrop-blur-sm border border-white/20 hover:border-white/40" aria-label="Toggle menu">
            ...
            {mobileMenuOpen ? <X className="relative w-5 h-5 transition-transform duration-300" /> : <Menu className="relative w-5 h-5 transition-transform duration-300" />}
          </button>
          
          {/* Desktop dropdown menu */}
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="hidden md:block relative p-2 sm:p-2.5 rounded-xl hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-white/50 focus:outline-none text-white transition-all duration-300 group backdrop-blur-sm border border-white/20 hover:border-white/40" aria-label="More options">
                <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            ...
            <DropdownMenuContent align="end" className="z-50 bg-white/95 backdrop-blur-md border-pink-200 shadow-xl min-w-[180px]">
              {!isAdmin && <DropdownMenuItem onClick={() => setShowLogin(true)} className="hover:bg-pink-50 focus:bg-pink-50 text-gray-700 font-medium py-3 px-4 cursor-pointer">
                  <Shield className="w-4 h-4 mr-2 text-pink-600" />
                  Admin Login
                </DropdownMenuItem>}
              {isAdmin && <>
                  <DropdownMenuItem onClick={() => navigate("/admin")} className="hover:bg-pink-50 focus:bg-pink-50 text-gray-700 font-medium py-3 px-4 cursor-pointer">
                    <Shield className="w-4 h-4 mr-2 text-pink-600" />
                    Admin Dashboard
                  </DropdownMenuItem>
                  <div className="border-t border-pink-100 my-1"></div>
                  <DropdownMenuItem onClick={logoutAdmin} className="hover:bg-red-50 focus:bg-red-50 text-red-600 font-medium py-3 px-4 cursor-pointer">
        ...
        {/* Mobile menu overlay */}
        {mobileMenuOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-md border-t border-pink-200 shadow-xl z-40">
