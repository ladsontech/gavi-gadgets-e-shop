  ...
  return <header className="w-full bg-white/80 backdrop-blur-lg border-b border-white/20 shadow-lg sticky top-0 z-50">
      ...
      <nav className="relative flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Mobile Menu Button */}
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2.5 rounded-xl hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-purple-200 transition-all duration-200 hover:shadow-md" aria-label="Toggle menu">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="hidden md:flex p-2.5 rounded-xl hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-purple-200 transition-all duration-200 hover:shadow-md" aria-label="More options">
                <MoreVertical className="w-5 h-5 text-gray-700" />
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56 bg-white/90 backdrop-blur-md border border-white/20 shadow-xl rounded-xl">
              {!isAdmin && <DropdownMenuItem onClick={() => setShowLogin(true)} className="flex items-center gap-3 px-4 py-3 hover:bg-purple-50/80 cursor-pointer rounded-lg mx-1 mt-1">
                  <Shield className="w-4 h-4 text-purple-600" />
                  <span className="text-gray-700 font-medium">Admin Login</span>
                </DropdownMenuItem>}
              {isAdmin && <>
                  <DropdownMenuItem onClick={() => navigate("/admin")} className="flex items-center gap-3 px-4 py-3 hover:bg-purple-50/80 cursor-pointer rounded-lg mx-1 mt-1">
                    <Shield className="w-4 h-4 text-purple-600" />
                    <span className="text-gray-700 font-medium">Admin Dashboard</span>
                  </DropdownMenuItem>
                  <div className="border-t border-gray-200/50 my-2 mx-2"></div>
                  <DropdownMenuItem onClick={logoutAdmin} className="flex items-center gap-3 px-4 py-3 hover:bg-red-50/80 cursor-pointer rounded-lg mx-1 mb-1">
        ...
        {/* Mobile Menu Overlay */}
        {mobileMenuOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-white/90 backdrop-blur-lg border-b border-white/20 shadow-xl rounded-b-xl">
